...
#
# README.md is referenced by pyproject.toml's `readme =` field, but it is
# not copied in this manifest-only layer. uv's build frontend stats the
# readme path during dep resolution, so we `touch` an empty placeholder —
# the real README is restored by `COPY . .` below.
#
