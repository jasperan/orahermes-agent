...
if is_termux; then
    ...
    # breaks; users keep voice / google / slack / matrix etc. even
    ...
    _ALL_EXTRAS=(
        ...
        pty mcp homeassistant sms acp voice dingtalk feishu google
        bedrock web youtube
    )
    _SAFE_EXTRAS=()
    ...
    if [ -f "uv.lock" ]; then
        ...
        # [matrix] (encrypted-room deps need make on Windows) and [rl] (git+https
