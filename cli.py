...
class HermesCLI:
    ...
    def __init__(
        ...
        Args:
            ...
            resume: Session ID to resume (restores conversation history from Oracle)
        ...
        # Initialize Oracle session store early so /title works before first message
        from hermes_state import SessionDB
        self._session_db = SessionDB()

        # Opportunistic Oracle session maintenance — runs at most once per
        # min_interval_hours, tracked via state_meta in Oracle so
        ...
        # /exit --delete: when True, the current session's Oracle history and
    ...
    def _init_agent(self, *, model_override: str = None, runtime_override: dict = None, request_overrides: dict | None = None) -> bool:
        ...
        When resuming a session, restores conversation history from Oracle.
        ...
        # Initialize Oracle session store for CLI sessions (if not already done in __init__)
    ...
    def _handle_snapshot_command(self, command: str):
        ...
        if subcmd in {"list", "ls"}:
            ...
            if restore_quick_snapshot(snap_id):
                ...
                print("  Restart recommended for session state changes to take effect.")
            else:
    ...
    def show_config(self):
        ...
        elif isinstance(self.api_key, str) and len(self.api_key) > 4:
            api_key_display = '********' + self.api_key[-4:]
        else:
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        Flow:
          ...
          4. Block-poll Oracle session state for terminal state (timeout 60s).
        ...
        # Make sure the session row exists in Oracle. Most CLI sessions
        ...
        try:
            ...
            _cprint(f"  Could not ensure Oracle session row: {exc}")
            return True
    ...
    def save_conversation(self):
        ...
        every message is already persisted incrementally to the Oracle session
    ...
    def process_command(self, command: str) -> bool:
        ...
        if canonical in {"quit", "exit"}:
            ...
            # session's transcripts + Oracle history. Ported from
    ...
    def run(self):
        ...
        try:
            ...
            if hasattr(self, '_session_db') and self._session_db and self.agent:
                ...
                # and Oracle history. Ported from google-gemini/gemini-cli#19332.
