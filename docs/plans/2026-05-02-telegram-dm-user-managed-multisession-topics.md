...
**Tech Stack:** Hermes gateway, Telegram Bot API 9.4+, python-telegram-bot adapter, Oracle-backed SessionDB / side tables, pytest.
...
- Use Oracle Database persistence, not local files or JSON sidecars.
...
Use Oracle Database, but topic-mode schema changes are **explicit opt-in migrations**, not automatic startup reconciliation.
...
- old/default Telegram behavior must keep working without creating local database files;
...
**Goal:** Add rollback-safe Oracle Database support for Telegram topic mode without creating local database files on ordinary upgrade/startup.
...
1. opening an old/current Oracle-backed `SessionDB()` does not create topic-mode tables or `sessions` origin columns;
...
**Goal:** Add Oracle Database persistence for activation and topic bindings.
