...
## Memory Providers: Oracle-Only

OraHermes only accepts Oracle Database-backed persistence. Legacy local memory
files, SQLite stores, and external/cloud memory providers are disabled in this
fork until they are ported to Oracle-backed storage.

The `MemoryProvider` ABC (`agent/memory_provider.py`) remains only as a
compatibility target for future Oracle-backed work. PRs that add a provider
directory under `plugins/memory/`, add a non-Oracle persistence dependency, or
re-enable `memory.provider` discovery will be closed with a pointer to port the
design to Oracle first.
...
├── hermes_state.py           # Oracle-only session database facade with session titles/search
...
| Oracle Database | Session and message database configured by `ORACLE_DSN`, `ORACLE_USER`, `ORACLE_PASSWORD` |
...
- **Session persistence**: All conversations are stored in Oracle Database through `hermes_state.py`, with searchable session metadata and unique session titles. JSON logs go to `~/.hermes/sessions/`.
...
"oracledb==3.4.2"
"oci-openai==1.1.0"
"example-pre1-package>=0.4.22,<0.5"
