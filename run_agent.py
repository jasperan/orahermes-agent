...
class AIAgent:
    ...
    def _get_session_db_for_recall(self):
        ...
        Most frontends pass ``session_db`` into ``AIAgent`` explicitly. OraHermes
        requires Oracle persistence, so initialization errors are propagated
        instead of falling back to an unindexed session.
        """
    ...
    def _ensure_db_session(self) -> None:
        ...
        try:
            ...
            # Transient failure. Keep _session_db alive —
    ...
    def _persist_session(self, messages: List[Dict], conversation_history: List[Dict] = None):
        """Save session state to both JSON log and Oracle on any exit path.
    ...
    def _flush_messages_to_session_db(self, messages: List[Dict], conversation_history: List[Dict] = None):
        """Persist any un-flushed messages to the Oracle session store.
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        try:
            ...
            # messages weren't fully written to Oracle — the resumed agent starts
    ...
    def _sync_external_memory_for_turn(
        ...
        *,
        ...
        """Mirror a completed turn into the optional memory manager.

        Called at the end of ``run_conversation`` with the cleaned user
        message (``original_user_message``) and the finalised assistant
        response. OraHermes currently leaves ``_memory_manager`` unset because
        non-Oracle memory providers are disabled; this method remains as a
        no-op compatibility hook for future Oracle-backed memory work.
        ...
        Normal completed turns still sync if a future Oracle-backed manager is
        installed. The body stays wrapped in ``try/except Exception`` so memory
        maintenance never blocks the user from seeing their response.
        """
        if interrupted:
            return
...
def main(
    ...
    try:
        # Direct ``hermes-agent`` / ``python run_agent.py`` invocations do not
        # pass through hermes_cli.main, so they must perform MCP discovery here.
        # This is safe in the direct entry point because no gateway event loop
        # is running on this thread.
        from tools.mcp_tool import discover_mcp_tools

        discover_mcp_tools()
    except Exception:
        logger.debug("MCP tool discovery failed for direct run_agent entry point", exc_info=True)
