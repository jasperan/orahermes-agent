...
config.yaml, .env, sessions, skills, gateway, cron, and logs.
...
# exclusion list (export drops logs / caches / legacy local state files because the
# archive is a portable snapshot; clone-all keeps runtime files because the
# cloned profile is meant to keep working immediately).
...
_LOCAL_STATE_SUFFIXES = (".db", ".db-shm", ".db-wal", ".db-journal")
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      ...
      2. Universal exclusions at any depth — Python bytecode caches,
         runtime sockets / temp files, and stale local state sidecars.
    ...
    def _ignore(directory: str, names: List[str]) -> List[str]:
        ...
        for entry in names:
            ...
            if (
                entry == "__pycache__"
                or entry.endswith((".pyc", ".pyo", ".sock", ".tmp", *_LOCAL_STATE_SUFFIXES))
            ):
...
# The default profile contains infrastructure (repo checkout, worktrees,
...
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    ...
    "memories",             # legacy local memory files; Oracle-only runtime ignores them
    # Runtime state
...
def create_profile(
    ...
    if clone_all and source_dir:
        ...
        if source_dir is not None:
            ...
            # Clone selected non-runtime subdirectory files.
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    items = [
        "All config, API keys, sessions, skills, cron jobs",
    ]
...
def _default_export_ignore(root_dir: Path):
    ...
    def _ignore(directory: str, contents: list) -> set:
        ...
        for entry in contents:
            ...
            if entry == "__pycache__" or entry.endswith((".sock", ".tmp", *_LOCAL_STATE_SUFFIXES)):
                ignored.add(entry)
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        shutil.copytree(
            ...
            ignore=lambda d, contents: {
                ...
                or entry == "memories"
                or entry.endswith(_LOCAL_STATE_SUFFIXES)
            },
        )
