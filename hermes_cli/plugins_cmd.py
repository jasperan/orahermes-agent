...
def _discover_memory_providers() -> list[tuple[str, str]]:
    """Return no memory providers in Oracle-only builds."""
    return []
...
def _save_memory_provider(name: str) -> None:
    """Clear memory.provider config in Oracle-only builds."""
    from hermes_cli.config import load_config, save_config
...
def _configure_memory_provider() -> bool:
    """Report the Oracle-only policy; no provider picker is available."""
    from rich.console import Console

    _save_memory_provider("")
    Console().print(
        "[yellow]Memory providers are disabled.[/yellow] "
        "OraHermes runtime persistence must use Oracle Database only."
    )
    return False
...
def _run_composite_ui(curses, plugin_names, plugin_labels, plugin_selected,
    ...
    def _current_category_value(category_name: str) -> str:
        if category_name == "Context Engine":
                ...
                if cursor < n_plugins:
                    ...
                    if 0 <= ci < n_categories:
                        ...
                        if changed:
                            ...
                            categories[ci] = (
                                _cat_name,
                                _current_category_value(_cat_name),
                                cat_fn,
                            )
                        # Re-enter curses
    ...
    if result_holder["providers_changed"]:
        ...
        console.print(
            f"[green]\u2713[/green] Context engine: [bold]{new_context}[/bold]"
        )
