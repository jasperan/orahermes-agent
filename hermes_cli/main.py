...
Usage:
    ...
    hermes memory status       Show Oracle-only memory-provider status
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # Snapshot critical local state (config, pairing JSONs, etc.)
...
""",
    ...
    backup_parser.add_argument(
        ...
        "--quick",
        action="store_true",
        help="Quick snapshot: only critical local state files (config, .env, auth, cron)",
    )
    backup_parser.add_argument(
    ...
    memory_parser = subparsers.add_parser(
        "memory",
        help="Show disabled memory-provider status",
        description=(
            "OraHermes disables file-backed memory and external memory "
            "providers. Runtime persistence must use Oracle Database only."
        ),
    )
    memory_sub = memory_parser.add_subparsers(dest="memory_command")
    memory_sub.add_parser(
        "setup", help="Disable memory providers and report Oracle-only status"
    )
    memory_sub.add_parser("status", help="Show Oracle-only memory status")
    memory_sub.add_parser("off", help="Disable memory provider config")
    _reset_parser = memory_sub.add_parser(
        "reset",
        help="Erase legacy local memory files (MEMORY.md and USER.md)",
    )
    ...
    sessions_parser = subparsers.add_parser(
        ...
        description="View and manage the Oracle session store",
    )
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            if dsn:
                print(f"Database backend: Oracle ({dsn})")
