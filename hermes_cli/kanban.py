...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser = parent_subparsers.add_parser(
        ...
        description=(
            "Kanban is disabled in OraHermes until the upstream board is "
            "ported to Oracle Database."
        ),
    )
    # --- global --board flag ---
    ...
    # --- init ---
    sub.add_parser("init", help="Disabled in OraHermes until Kanban is Oracle-backed")
...
def kanban_command(args: argparse.Namespace) -> int:
    ...
    # Boards management doesn't touch persistent storage at all — dispatch
    # early so fresh installs without an Oracle-backed board can still use
    ...
    # Auto-initialize storage before dispatching any subcommand. In
    # OraHermes this currently raises a clear disabled error until the board
    # has an Oracle-backed implementation.
    try:
        kb.init_db()
...
def _dispatch_boards(args: argparse.Namespace) -> int:
    ...
    Boards management is disabled in OraHermes until the upstream board is
    ported to Oracle Database.
    """
