...
_EXCLUDED_SUFFIXES = (
    ".pyc",
    ".pyo",
    # OraHermes is Oracle-only. Local state sidecars are excluded instead of
    # snapshotted so stale upstream stores are not carried forward.
    ".db",
    ".db-wal",
    ".db-shm",
    ".db-journal",
...
_SECRET_FILE_NAMES = {".env", "auth.json"}
...
# platform-specific JSON blobs, so it's listed here explicitly. `hermes update`
# snapshots this set before pulling so approved-user lists are recoverable if
# anything goes wrong (issue #15733).
_QUICK_STATE_FILES = (
    "config.yaml",
    ".env",
    "auth.json",
    ...
    "processes.json",
    # Pairing stores (generic + per-platform JSONs)
...
def _write_full_zip_backup(out_path: Path, hermes_root: Path) -> Optional[Path]:
    ...
    Uses the same exclusion rules as :func:`run_backup`.
...
def create_pre_update_backup(
    ...
    Mirrors :func:`run_backup` (same exclusion rules)
...
def create_pre_migration_backup(
    ...
    ``_write_full_zip_backup`` — same exclusions, restorable with
    ``hermes import <archive>``.  Writes to
