...
* User-owned paths (memories/, sessions/, legacy DB files, auth.json, .env,
...
def _is_user_owned_excluded(name: str) -> bool:
    return name in USER_OWNED_EXCLUDE or name.endswith(_LOCAL_STATE_SUFFIXES)
...
def _copy_dist_payload(
    ...
    for entry in staged.iterdir():
        ...
        if _is_user_owned_excluded(name):
            continue
        ...
        if entry.is_dir():
            ...
            shutil.copytree(
                entry,
                dest,
                ignore=lambda d, names: [n for n in names if _is_user_owned_excluded(n)],
            )
        else:
