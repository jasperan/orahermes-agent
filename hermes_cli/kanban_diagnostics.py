...
def _task_field(task, name, default=None):
    ...
    Callers pass row-like mappings, kanban_db.Task dataclasses (attribute
    access), or plain dicts. This normalises them so rule functions don't
    have to branch on type each time.
    """
    if task is None:
        return default
    # Row-like mappings and plain dicts both support mapping access.
    try:
