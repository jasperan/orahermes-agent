...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["hermes-cli"],
    "storage": {
        # OraHermes supports Oracle Database only. This is informational for
        # config/status surfaces; there is no alternate DB backend.
        "backend": "oracle",
    },
    "agent": {
        "max_turns": 90,
    ...
    # Persistent memory -- disabled in OraHermes until it has Oracle-backed
    # storage. The old file-backed MEMORY.md/USER.md store persists data
    # outside Oracle, which violates the Oracle-only runtime contract.
    "memory": {
        "memory_enabled": False,
        "user_profile_enabled": False,
        "memory_char_limit": 2200,   # ~800 tokens at 2.75 chars/token
        "user_char_limit": 1375,     # ~500 tokens at 2.75 chars/token
        # External memory providers are ignored in OraHermes until ported to
        # Oracle Database.
        "provider": "",
    },
    ...
    # Kanban multi-agent coordination. Disabled in OraHermes until the
    # upstream local board store is ported to Oracle Database.
    "kanban": {
    ...
    # Oracle session storage — controls automatic cleanup of ended sessions.
    "sessions": {
        ...
        "retention_days": 90,
        # Kept for config compatibility. Oracle-backed storage ignores this.
        "vacuum_after_prune": True,
        # Minimum hours between auto-maintenance runs (avoids repeating
        # the sweep on every CLI invocation). Tracked in Oracle state_meta
        # so it's shared across all processes.
        "min_interval_hours": 24,
    },
...
ENV_VARS_BY_VERSION: Dict[int, List[str]] = {
    ...
    24: ["ORACLE_DSN", "ORACLE_USER", "ORACLE_PASSWORD"],
}
...
OPTIONAL_ENV_VARS = {
    # ── Oracle storage (required for OraHermes persistence) ──
    "ORACLE_DSN": {
        "description": "Oracle Database DSN for OraHermes session storage",
        "prompt": "Oracle DSN (for example localhost:1521/FREEPDB1)",
        "url": None,
        "password": False,
        "category": "storage",
    },
    "ORACLE_USER": {
        "description": "Oracle Database user for OraHermes session storage",
        "prompt": "Oracle user",
        "url": None,
        "password": False,
        "category": "storage",
    ...
    "ORACLE_PASSWORD": {
        "description": "Oracle Database password for OraHermes session storage",
