...
def _termux_install_all_fallback_notes() -> list[str]:
    return [
        ...
        "Matrix E2EE is disabled in OraHermes until the crypto store is Oracle-backed.",
...
def run_doctor(args):
    ...
    # OraHermes keeps runtime persistence Oracle-only. Legacy file-backed
    # memory may exist from older installs, but it is no longer created or used.
    memories_dir = hermes_home / "memories"
    if memories_dir.exists():
        check_info(f"{_DHH}/memories/ directory exists (legacy local memory is disabled)")
        memory_file = memories_dir / "MEMORY.md"
        user_file = memories_dir / "USER.md"
        if memory_file.exists():
            size = len(memory_file.read_text(encoding="utf-8").strip())
            check_info(f"MEMORY.md exists ({size} chars; ignored by Oracle-only runtime)")
        if user_file.exists():
            size = len(user_file.read_text(encoding="utf-8").strip())
            check_info(f"USER.md exists ({size} chars; ignored by Oracle-only runtime)")
    else:
        check_ok("Legacy file-backed memory disabled", "(Oracle-only runtime)")
    
    # Check Oracle session store
    oracle_missing = [
        name
    ...
    if oracle_missing:
        check_warn(
            "Oracle session database is not configured",
            f"(missing {', '.join(oracle_missing)}; apply oracle_setup.sql after configuring)",
        )
        issues.append("Oracle session database is not configured")
    else:
        try:
            ...
            try:
                ...
                db.close()
            check_ok("Oracle session database reachable")
        except Exception as e:
            check_warn(f"Oracle session database is configured but unreachable: {e}")
            issues.append("Oracle session database is configured but unreachable")
    ...
    if not _active_memory_provider:
        check_ok("Memory providers disabled", "(Oracle-only runtime)")
    else:
        check_warn(
            "Memory provider configured but ignored",
            "OraHermes disables non-Oracle memory providers",
        )
