...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "memory.provider": {
        "type": "select",
        "description": "Memory providers are disabled in OraHermes; persistence is Oracle-only",
        "options": [""],
    },
    "approvals.mode": {
        "type": "select",
...
async def get_usage_analytics(days: int = 30):
    ...
    try:
        ...
        daily = db.query_dicts("""
            ...
            FROM (
                SELECT TO_CHAR(DATE '1970-01-01' + (started_at / 86400), 'YYYY-MM-DD') as day,
        ...
        by_model = db.query_dicts("""
            SELECT model,
                   ...
                   COUNT(*) as sessions,
                   SUM(COALESCE(api_call_count, 0)) as api_calls
            FROM sessions WHERE started_at > :cutoff AND model IS NOT NULL
...
async def serve_plugin_asset(request: Request, plugin_name: str, file_path: str):
    ...
    if suffix not in content_types:
        raise HTTPException(status_code=403, detail="Unsupported plugin asset type")
