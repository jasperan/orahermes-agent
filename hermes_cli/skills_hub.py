...
Powers both:
  - `orahermes skills <subcommand>` (CLI argparse entry point)
...
def do_search(query: str, source: str = "all", limit: int = 10,
    ...
    c.print(table)
    c.print("[dim]Use: orahermes skills inspect <identifier> to preview, "
            "orahermes skills install <identifier> to install[/]\n")
...
def do_inspect(identifier: str, console: Optional[Console] = None) -> None:
    ...
    if bundle and "SKILL.md" in bundle.files:
        ...
        c.print(Panel(preview, title="SKILL.md Preview", subtitle="orahermes skills install <id> to install"))
...
def do_tap(action: str, repo: str = "", console: Optional[Console] = None) -> None:
    ...
    if action == "list":
        ...
        if not repo:
            c.print("[bold red]Error:[/] Repo required. Usage: orahermes skills tap add owner/repo\n")
            return
        if mgr.add(repo):
        ...
        if not repo:
            c.print("[bold red]Error:[/] Repo required. Usage: orahermes skills tap remove owner/repo\n")
            return
        if mgr.remove(repo):
    ...
    if target == "github":
        if not repo:
            c.print("[bold red]Error:[/] --repo required for GitHub publish.\n"
                    "Usage: orahermes skills publish <path> --to github --repo owner/repo\n")
            return
...
def skills_command(args) -> None:
    """Router for `orahermes skills <subcommand>` — called from hermes_cli/main.py."""
    ...
    if action == "search":
        ...
        if snap_action == "export":
            ...
            _console.print("Usage: orahermes skills snapshot [export|import]\n")
    elif action == "tap":
        tap_action = getattr(args, "tap_action", None)
        repo = getattr(args, "repo", "") or getattr(args, "name", "")
        if not tap_action:
            _console.print("Usage: orahermes skills tap [list|add|remove]\n")
            return
        do_tap(tap_action, repo=repo)
    else:
        _console.print("Usage: orahermes skills [search|install|inspect|list|audit|uninstall|publish|snapshot|tap]\n")
        _console.print("Run 'orahermes skills <command> --help' for details.\n")
