...
/var/lib/hermes/                    ──►  /data/          (rw)
  ...
  │   ├── sessions/, memories/             (legacy/runtime artifacts; Oracle holds session state)
...
For optional extras already declared in hermes-agent's `pyproject.toml`, use `extraDependencyGroups` to include them in the sealed venv at build time:
...
This is resolved by uv alongside core dependencies in a single pass — no PYTHONPATH patching, no collision risk. Available groups match the `[project.optional-dependencies]` keys in `pyproject.toml` (e.g., `"voice"`, `"matrix"`, `"bedrock"`).
