...
| Command | Description |
|---------|-------------|
| `/quit` | Exit the CLI (also: `/exit`). See note on `/q` under `/queue` above. Pass `--delete` (or `-d`) — e.g. `/exit --delete` — to also permanently remove the current session's Oracle history and on-disk transcripts before exiting. Useful for privacy-sensitive or one-off tasks. |
