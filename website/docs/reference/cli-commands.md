...
| `hermes memory` | Show or clear disabled memory-provider configuration. OraHermes uses Oracle Database only. |
...
Kanban is disabled in OraHermes until its board schema is ported to Oracle Database. The command surface is retained so existing scripts fail closed with a clear message.
...
| Action | Purpose |
|--------|---------|
| `init` | Disabled in OraHermes until Kanban is Oracle-backed. |
...
| `-q`, `--quick` | Quick snapshot: only critical config files, auth, and cron jobs. Much faster than a full backup. |
| `-l`, `--label <name>` | Label for the snapshot (only used with `--quick`). |

OraHermes runtime state lives in Oracle Database and is not copied through local database file snapshots.

**What's excluded from the zip:**

- Local database sidecars are ignored because OraHermes does not use local runtime database files.
...
Show or clear disabled memory-provider configuration. OraHermes disables local file-backed memory and external memory providers; runtime persistence must use Oracle Database only.
...
External memory provider commands are disabled in OraHermes. Runtime persistence must use Oracle Database only.
:::
