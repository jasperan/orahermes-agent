...
├── hermes_state.py           # Oracle-backed session/state facade
...
7. **[Session Storage](./session-storage.md)** — Oracle-backed sessions, messages, and lineage
...
- **`prompt_builder.py`** — Assembles the system prompt from: personality (SOUL.md), skills, context files (AGENTS.md, .hermes.md), tool-use guidance, and model-specific instructions
...
Oracle-backed session storage. Sessions have lineage tracking (parent/child across compressions), per-platform isolation, and fail-fast startup when Oracle is unavailable.
