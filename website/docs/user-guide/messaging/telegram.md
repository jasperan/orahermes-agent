...
| Persistence | `extra.dm_topics` in config | Oracle-backed topic binding tables |
...
- Activation persists to `telegram_dm_topic_mode(chat_id, user_id, enabled, ...)` in Oracle
...
- The topic-mode schema is Oracle-backed in OraHermes.
...
Use `/topic off` for cleanup. Direct database edits should be reserved for
operators who manage the Oracle schema.

### Downgrading Hermes

If you downgrade to a Hermes version that predates `/topic`, the feature simply stops working. Re-upgrading reactivates multi-session mode from the Oracle-backed topic bindings.
