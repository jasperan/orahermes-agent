...
:::note
The git repo contains **everything in the profile directory except things already excluded from distributions**: `auth.json`, `.env`, legacy `memories/`, transcript `sessions/`, logs, workspace data, caches, and local runtime files. Those stay on your machine. You can also add a `.gitignore` if you want to exclude additional paths.
:::
...
| **User-owned** | legacy `memories/`, transcript `sessions/`, `auth.json`, `.env`, `logs/`, `workspace/`, `plans/`, `home/`, `*_cache/`, `local/` | Never touched |
...
- Oracle-backed session metadata is not stored in profile git distributions
