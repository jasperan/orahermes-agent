...
Resuming restores the full conversation history from Oracle Database. The agent sees all previous messages, tool calls, and responses — just as if you never left.
...
CLI sessions are stored in OraHermes's Oracle-backed session store. The database keeps:
...
Some messaging adapters also keep per-platform transcript files alongside the database, but the CLI itself resumes from the Oracle session store.
