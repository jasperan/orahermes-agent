...
Karakeep, Open WebUI, or anything that speaks OpenAI-compatible chat
completions use your Hermes-managed provider subscription as their
...
External memory systems are not part of OraHermes runtime persistence. The
proxy can still serve generic OpenAI-compatible clients, but OraHermes itself
stores runtime state in Oracle Database only.
