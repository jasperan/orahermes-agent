...
with oracledb.connect(
    ...
    (n,) = conn.cursor().execute(
        "SELECT COUNT(*) FROM messages WHERE ts > SYSTIMESTAMP - INTERVAL '2' HOUR"
    ).fetchone()

print(json.dumps({"wakeAgent": bool(n), "context": {"new_rows": n}}))
```
...
:::tip
OraHermes runtime state is Oracle-backed. Prefer supported tools over direct schema queries unless you own the schema being queried.
:::
