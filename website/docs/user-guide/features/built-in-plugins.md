...
Memory providers are disabled in OraHermes unless they are ported to Oracle Database. Context engines remain managed through `hermes plugins`. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
- Scans your Oracle-backed session history on the dashboard backend
