...
def pick_longest_session() -> str:
    from hermes_state import SessionDB

    db = SessionDB()
    try:
        sessions = db.list_sessions_rich(limit=200, include_children=True)
    finally:
        db.close()
    if not sessions:
        sys.exit("no sessions in Oracle session database")
    sessions.sort(key=lambda row: int(row.get("message_count") or 0), reverse=True)
...
def run_once(args: argparse.Namespace) -> dict[str, Any]:
    ...
    sid = args.session or pick_longest_session()
    print(f"• session: {sid}")
