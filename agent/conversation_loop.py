...
def _restore_or_build_system_prompt(agent, system_message, conversation_history):
    ...
    # Persist the system prompt snapshot in the Oracle session store. Failure here used
...
def run_conversation(
    ...
    if (
        ...
        if _preflight_tokens >= agent.context_compressor.threshold_tokens:
            ...
            for _pass in range(3):
                ...
                # compressed messages to the new Oracle session row, not
                ...
                if hasattr(response, 'usage') and response.usage:
                    ...
                    if agent._session_db and agent.session_id:
                        try:
                            ...
                            # _ensure_db_session() may have failed because the
                            # Oracle session store was temporarily unavailable.
                            # Retry here so per-call token deltas are
    ...
    # Persist session to both JSON log and Oracle only after private retry
