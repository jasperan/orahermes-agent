"""Compatibility base class for future Oracle-backed memory providers.

OraHermes disables legacy local-file and external memory providers because
runtime persistence must stay Oracle-only.  The abstract interface remains so
Oracle-backed provider work has a stable target, but current runtime discovery
returns no providers and ``memory.provider`` config is ignored.

Registration:
  Provider discovery is disabled in ``plugins/memory`` until a provider is
