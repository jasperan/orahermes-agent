...
def init_agent(
    ...
    # Oracle session store (optional -- provided by CLI or gateway)
    ...
    # Persistent memory is disabled in OraHermes until it is backed by Oracle.
    # The legacy MEMORY.md/USER.md store and external memory providers persist
    # state outside Oracle, which violates the Oracle-only runtime contract.
    ...
    if not skip_memory and isinstance(mem_config, dict):
        if (
            mem_config.get("memory_enabled")
            or mem_config.get("user_profile_enabled")
            or str(mem_config.get("provider", "")).strip()
        ):
            _ra().logger.warning(
                "Persistent memory is disabled: OraHermes only supports Oracle-backed runtime persistence."
            )

    # Memory provider plugins are also disabled for the same Oracle-only
    # reason. Keep the attribute present so lifecycle hooks remain no-ops.
