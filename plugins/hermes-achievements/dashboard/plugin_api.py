...
def scan_sessions(
    ...
    unreachable. ``None`` and non-positive values request the full catalog.
    ...
    # SessionDB treats non-positive limits as "no practical limit", so the
    # full session history flows through unless the caller explicitly requests
    # a small sample (e.g. a smoke test).
    db_limit = 0 if (limit is None or limit <= 0) else int(limit)
