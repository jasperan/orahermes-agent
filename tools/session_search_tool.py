...
mode parameter):
  ...
  1. DISCOVERY — pass ``query``. Runs Oracle-backed search, dedupes hits by session lineage,
  ...
  2. SCROLL — pass ``session_id`` + ``around_message_id``. Returns a window
     of ±window messages centered on the anchor, no keyword search, no bookends. To
...
All three modes operate on the Oracle session database and the
get_anchored_view / get_messages_around primitives in hermes_state.
...
def _list_recent_sessions(db, limit: int, current_session_id: str = None) -> str:
    """Return metadata for the most recent sessions (no LLM calls, no keyword search)."""
    try:
...
def _scroll(
    ...
    No keyword search, no bookends — just the slice. The discovery shape's lineage
    ...
    if not messages:
        owning = None
        try:
            find_message_session = getattr(db, "find_message_session", None)
            if callable(find_message_session):
                owning = find_message_session(around_message_id)
        except Exception as e:
...
def _discover(
    ...
    """Discovery shape: Oracle search + anchored window + bookends per hit."""
    ...
    try:
        ...
        logging.error("Oracle session search failed: %s", e, exc_info=True)
...
def check_session_search_requirements() -> bool:
    """Requires Oracle session database configuration."""
    import os

    return all(os.environ.get(name) for name in ("ORACLE_DSN", "ORACLE_USER", "ORACLE_PASSWORD"))


SESSION_SEARCH_SCHEMA = {
    "name": "session_search",
    "description": (
        "Search past sessions stored in Oracle, or scroll inside one. "
        "Oracle-backed retrieval over the message store. No LLM calls — every "
        "shape returns actual messages from the DB.\n\n"
        "THREE CALLING SHAPES\n\n"
        "  1) DISCOVERY — pass `query`:\n"
        "     session_search(query=\"auth refactor\", limit=3)\n"
        "     Runs Oracle-backed search, dedupes hits by session lineage, returns the top N sessions. "
        "Each result carries:\n"
        ...
        "     Returns a window of ±`window` messages centered on the anchor. No keyword search, "
        ...
        "QUERY SYNTAX\n\n"
        "  Plain text queries use Oracle-backed matching over stored message content. "
        "Use concise terms or quoted phrases for best recall.\n\n"
        "WHEN TO USE\n\n"
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "sort": {
                ...
                "enum": ["newest", "oldest"],
                "description": (
                    "Discovery shape only. Temporal bias on top of Oracle search ranking. Omit "
