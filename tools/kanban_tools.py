...
These tools are disabled in OraHermes until the upstream board is ported to
Oracle Database. A normal ``hermes chat`` session sees **zero** kanban tools in
its schema.

Why tools instead of just shelling out to ``hermes kanban``?

1. **Backend portability.** Worker handoff needs a shared durable store. The
   upstream local board is not allowed in OraHermes, so this toolset remains
   hidden until an Oracle-backed board exists.
