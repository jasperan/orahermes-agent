...
# OraHermes is Oracle-only. These upstream suites still exercise local
# SessionDB(db_path=...), sqlite state.db/kanban.db, or file-backed session
# stores. Keep the normal test tree active, but do not collect these legacy
# local-persistence files until they are ported to Oracle-backed fixtures.
_ORACLE_INCOMPATIBLE_TESTS = {
...
def pytest_ignore_collect(collection_path, config):
    if config.getoption("--orahermes-include-legacy-local-db", default=False):
        return False
    ...
    try:
        ...
        return False
    return rel in _ORACLE_INCOMPATIBLE_TESTS
...
def pytest_collection_modifyitems(config, items):
    ...
    for item in items:
        try:
            rel = Path(str(item.fspath)).resolve().relative_to(PROJECT_ROOT.resolve()).as_posix()
        except ValueError:
            kept.append(item)
            continue
        if rel in _ORACLE_INCOMPATIBLE_TESTS:
...
def pytest_addoption(parser):
    parser.addoption(
        "--orahermes-include-legacy-local-db",
        action="store_true",
        default=False,
        help="Collect legacy upstream local SQLite/Kanban tests that OraHermes disables.",
    )
