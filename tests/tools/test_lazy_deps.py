...
class TestAllowlist:
    ...
    def test_feature_install_command_returns_pip_invocation(self):
        cmd = ld.feature_install_command("search.firecrawl")
        assert cmd is not None
        assert cmd.startswith("uv pip install")
        assert "firecrawl-py" in cmd
...
class TestSecurityGating:
    def test_disabled_via_config_raises(self, monkeypatch):
        # Pretend a lazy dependency is missing AND lazy installs are disabled.
...
class TestIsSatisfiedVersionAware:
    ...
    def test_exact_pin_match_returns_true(self, monkeypatch):
        self._fake_version(monkeypatch, {"examplepkg": "2.0.1"})
        assert ld._is_satisfied("examplepkg==2.0.1") is True

    def test_exact_pin_mismatch_returns_false(self, monkeypatch):
        # Installed 2.0.0, spec requires 2.0.1 → False (needs upgrade).
        self._fake_version(monkeypatch, {"examplepkg": "2.0.0"})
        assert ld._is_satisfied("examplepkg==2.0.1") is False
...
class TestActiveFeatures:
    ...
    def test_finds_features_with_at_least_one_package_installed(self, monkeypatch):
        # Pretend only slack-sdk is installed; nothing else.
        monkeypatch.setattr(
            ld, "_is_present",
            lambda spec: ld._pkg_name_from_spec(spec) == "slack-sdk",
        )
        active = ld.active_features()
        assert "platform.slack" in active
        # Oracle-only builds expose no lazy-installable memory providers.
        assert all(not feature.startswith("memory.") for feature in active)
