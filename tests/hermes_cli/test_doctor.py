...
class TestDoctorToolAvailabilityOverrides:
    def test_leaves_non_oracle_memory_provider_unavailable(self):
        provider_entry = {"name": "external_memory", "env_vars": [], "tools": ["query_user_context"]}
        ...
        assert available == []
        assert unavailable == [provider_entry]
...
class TestDoctorMemoryProviderSection:
    ...
    def test_no_provider_shows_oracle_only_ok(self, monkeypatch, tmp_path):
        out = self._run_doctor_and_capture(monkeypatch, tmp_path, provider="")
        assert "Memory Provider" in out
        assert "Memory providers disabled" in out
        assert "Oracle-only runtime" in out

    def test_external_provider_config_is_ignored(self, monkeypatch, tmp_path):
        out = self._run_doctor_and_capture(monkeypatch, tmp_path, provider="external")
        assert "Memory Provider" in out
        assert "Memory provider configured but ignored" in out
        assert "OraHermes disables non-Oracle memory providers" in out

    def test_mem0_provider_config_is_ignored(self, monkeypatch, tmp_path):
...
def test_run_doctor_termux_treats_docker_and_browser_warnings_as_expected(monkeypatch, tmp_path):
    ...
    assert "Matrix E2EE is disabled in OraHermes until the crypto store is Oracle-backed" in out
