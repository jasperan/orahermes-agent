...
class TestInstallHangupProtection:
    ...
    def test_wraps_stdout_and_stderr_with_mirror(self, tmp_path, monkeypatch):
        live_main = importlib.import_module("hermes_cli.main")
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        ...
        prev_out, prev_err = sys.stdout, sys.stderr
        state = live_main._install_hangup_protection(gateway_mode=False)

        try:
            # On Windows (no SIGHUP) we still wrap stdio and create the log.
            assert state["installed"] is True
            assert isinstance(sys.stdout, live_main._UpdateOutputStream)
            assert isinstance(sys.stderr, live_main._UpdateOutputStream)
            ...
            live_main._finalize_update_output(state)
