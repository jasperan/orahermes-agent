...
class TestNewEndpoints:
    ...
    def test_analytics_usage(self):
        with patch("hermes_state.SessionDB", return_value=_AnalyticsDB()):
    ...
    def test_analytics_usage_includes_skill_breakdown(self):
        ...
        with patch("hermes_state.SessionDB", return_value=_AnalyticsDB()), \
             patch("agent.insights.InsightsEngine.generate", return_value={"skills": skills}):
            resp = self.client.get("/api/analytics/usage?days=7")
        assert resp.status_code == 200

        data = resp.json()
        assert data["skills"]["summary"] == skills["summary"]
...
class TestPluginAPIAuth:
    ...
    def test_plugin_asset_requires_auth(self, monkeypatch, tmp_path):
        """Dashboard plugin static assets are not public files."""
        from hermes_cli import web_server

        plugin_dir = tmp_path / "plugins" / "asset-test" / "dashboard"
        plugin_dir.mkdir(parents=True)
        (plugin_dir / "index.js").write_text("export default 1;", encoding="utf-8")
        monkeypatch.setattr(
            web_server,
            "_get_dashboard_plugins",
            lambda *a, **k: [{"name": "asset-test", "_dir": str(plugin_dir)}],
        )

        resp = self.client.get("/dashboard-plugins/asset-test/index.js")
        assert resp.status_code == 401

        resp = self.auth_client.get("/dashboard-plugins/asset-test/index.js")
        assert resp.status_code == 200

    def test_plugin_asset_rejects_server_code(self, monkeypatch, tmp_path):
        """Only frontend static extensions can be served from dashboard plugins."""
        from hermes_cli import web_server

        plugin_dir = tmp_path / "plugins" / "asset-test" / "dashboard"
        plugin_dir.mkdir(parents=True)
        (plugin_dir / "plugin_api.py").write_text("SECRET = 'x'", encoding="utf-8")
        monkeypatch.setattr(
            web_server,
            "_get_dashboard_plugins",
            lambda *a, **k: [{"name": "asset-test", "_dir": str(plugin_dir)}],
        ...
        resp = self.auth_client.get("/dashboard-plugins/asset-test/plugin_api.py")
