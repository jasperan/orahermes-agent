...
class TestProviderDiscovery:
    ...
    def test_save_memory_provider(self, tmp_path, monkeypatch):
        """Saving a memory provider clears config in Oracle-only builds."""
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        config_file = tmp_path / "config.yaml"
        config_file.write_text("memory:\n  provider: legacy\n  memory_enabled: true\n  user_profile_enabled: true\n")
        from hermes_cli.plugins_cmd import _save_memory_provider
        _save_memory_provider("legacy")
        content = yaml.safe_load(config_file.read_text())
        assert content["memory"]["provider"] == ""
        assert content["memory"]["memory_enabled"] is False
        assert content["memory"]["user_profile_enabled"] is False
    ...
    def test_discover_memory_providers_empty(self):
        """Discovery is disabled in the Oracle-only runtime."""
