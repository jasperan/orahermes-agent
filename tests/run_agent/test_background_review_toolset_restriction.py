...
def test_background_review_installs_thread_local_whitelist():
    ...
    # OraHermes disables the legacy memory tool until it is Oracle-backed, so
    # the background review whitelist is skill-only unless memory is reenabled.
...
def test_background_review_agent_tools_are_limited():
    """Verify the resolved memory+skills toolsets only contain safe review tools.
