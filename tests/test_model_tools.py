...
class TestToolDefinitionCache:
    def test_quiet_cache_recomputes_after_check_fn_invalidation(self):
        state = {"available": False}
        tool_name = "tmp_model_tools_cache_probe"
        toolset = "tmp_model_tools_cache_probe"
        ...
        registry.register(
            ...
            toolset=toolset,
            schema={
                "description": "temporary cache probe",
        ...
        try:
            ...
            unavailable = get_tool_definitions([toolset], quiet_mode=True)
            assert [t["function"]["name"] for t in unavailable] == []

            state["available"] = True
            invalidate_check_fn_cache()
            available = get_tool_definitions([toolset], quiet_mode=True)
            assert [t["function"]["name"] for t in available] == [tool_name]
        finally:
            registry.deregister(tool_name)
            _clear_tool_defs_cache()
            invalidate_check_fn_cache()
