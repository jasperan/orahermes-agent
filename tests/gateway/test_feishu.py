...
class TestAdapterBehavior(unittest.TestCase):
    ...
    def test_download_remote_document_blocks_private_redirect(self):
        """Document/GIF downloads must re-check redirect targets for SSRF."""
        from gateway.config import PlatformConfig
        from gateway.platforms.feishu import FeishuAdapter
        ...
        class _FakeAsyncClient:
            ...
            async def get(self, *_a: object, **_k: object) -> object:
                ...
                for hook in self._hooks:
                    await hook(redirect)
                raise AssertionError("redirect guard should have blocked")

        def _is_safe(url: object) -> bool:
            return "169.254.169.254" not in str(url)

        adapter = FeishuAdapter(PlatformConfig())

        async def _run() -> None:
            with patch("tools.url_safety.is_safe_url", side_effect=_is_safe):
                with patch("httpx.AsyncClient", _FakeAsyncClient):
