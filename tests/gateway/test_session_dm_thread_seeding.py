...
class FakeOracleSessionDB:
    """Small Oracle SessionDB stand-in for session-thread unit tests."""
    ...
    def set_meta(self, key, value):
        assert key == GATEWAY_SESSION_INDEX_META_KEY
        self.meta[key] = value

    def create_session(self, session_id, **_kwargs):
    ...
    def append_message(self, session_id, role, content, **kwargs):
        ...
        message.update({key: value for key, value in kwargs.items() if value is not None})
...
@pytest.fixture()
def store(tmp_path):
    """SessionStore backed by fake Oracle metadata, with no local DB files."""
    config = GatewayConfig()
    s = SessionStore(sessions_dir=tmp_path, config=config)
    s._db = FakeOracleSessionDB()
    s._db_error = None
    s._loaded = False
    return s
