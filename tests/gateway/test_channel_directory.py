...
_ORACLE_SESSION_INDEX_BY_HOME = {}


@pytest.fixture(autouse=True)
def _fake_oracle_session_index(monkeypatch):
    """Route channel-directory session discovery through fake Oracle metadata."""
    ...
    class FakeSessionDB:
        ...
        def get_meta(self, _key):
            return json.dumps(_ORACLE_SESSION_INDEX_BY_HOME.get(self.home, {}))

        def close(self):
            return None

    _ORACLE_SESSION_INDEX_BY_HOME.clear()
    monkeypatch.setattr("hermes_state.SessionDB", FakeSessionDB)
    yield
    _ORACLE_SESSION_INDEX_BY_HOME.clear()


def _write_oracle_session_index(tmp_path, sessions_data):
    _ORACLE_SESSION_INDEX_BY_HOME[str(tmp_path)] = sessions_data
...
class TestBuildFromSessions:
    def _write_sessions(self, tmp_path, sessions_data):
        """Write fake Oracle session-index metadata for _build_from_sessions."""
        _write_oracle_session_index(tmp_path, sessions_data)

    def test_builds_from_oracle_session_index(self, tmp_path):
...
class TestBuildSlack:
    ...
    def test_no_team_clients_falls_back_to_sessions(self, tmp_path):
        _write_oracle_session_index(tmp_path, {
