...
- Session storage (conversations persisted to Oracle)
...
GATEWAY_SESSION_INDEX_META_KEY = "gateway.sessions.index"
...
class SessionEntry:
    ...
    # agent). Persisted to the Oracle session-index metadata so the flag
    # survives gateway restarts and prevents redundant finalization runs.
...
class SessionStore:
    ...
    Uses Oracle-backed SessionDB for session metadata, session-key routing,
    and message transcripts. OraHermes intentionally has no local transcript
    or session-index fallback.
    """
    ...
    def __init__(self, sessions_dir: Path, config: GatewayConfig,
        ...
        # Initialize Oracle session database. Construction is allowed to finish
        # so tests and gateway status objects can be built, but every storage
        # operation below calls _require_db() and fails closed if Oracle is not
        # configured.
        self._db = None
        ...
        try:
            ...
            self._db_error = e
            logger.error("Oracle session store unavailable; gateway session persistence is disabled: %s", e)

    def _require_db(self):
        """Return the Oracle SessionDB or fail closed when it is unavailable."""
        if self._db is not None:
            return self._db
        detail = f": {self._db_error}" if self._db_error else ""
        raise RuntimeError(
            "Oracle session store unavailable; OraHermes does not support "
            f"local session persistence{detail}"
        )
    ...
    def _ensure_loaded_locked(self) -> None:
        """Load the session-key index from Oracle. Must hold self._lock."""
        if self._loaded:
            return
                ...
                if isinstance(data, dict):
                    for key, entry_data in data.items():
                        if not isinstance(entry_data, dict):
                            continue
                        try:
                            self._entries[str(key)] = SessionEntry.from_dict(entry_data)
                        except (ValueError, KeyError, TypeError):
                            # Skip entries with unknown/removed platform values.
                            continue
            except Exception as e:
                raise RuntimeError("Failed to load Oracle gateway session index") from e

        self._loaded = True
    
    def _save(self) -> None:
        """Save the session-key index to Oracle metadata."""
        db = self._require_db()
        data = {key: entry.to_dict() for key, entry in self._entries.items()}
        db.set_meta(GATEWAY_SESSION_INDEX_META_KEY, json.dumps(data, ensure_ascii=False))
    ...
    def has_any_sessions(self) -> bool:
        ...
        Uses Oracle as the source of truth because it preserves
        ...
        """
        return self._require_db().session_count() > 1
    ...
    def get_or_create_session(
        ...
        Creates a session record in Oracle when a new session starts.
        """
        session_key = self._generate_session_key(source)
        now = _now()

        # Database calls are made outside the lock to avoid holding it during I/O.
        ...
        # Database operations outside the lock
        if self._db and db_end_session_id:
            try:
                self._db.end_session(db_end_session_id, "session_reset")
            except Exception as e:
                raise RuntimeError("Failed to end Oracle session during reset") from e

        if self._db and db_create_kwargs:
            try:
                self._db.create_session(**db_create_kwargs)
            except Exception as e:
                raise RuntimeError("Failed to create Oracle gateway session") from e
    ...
    def prune_old_entries(self, max_age_days: int) -> int:
        ...
        the transcript in Oracle stays, but the session_key → session_id
    ...
    def reset_session(self, session_key: str, display_name: Optional[str] = None) -> Optional[SessionEntry]:
        ...
        if self._db and db_end_session_id:
            try:
                ...
                raise RuntimeError("Failed to end Oracle session during manual reset") from e

        if self._db and db_create_kwargs:
            try:
                self._db.create_session(**db_create_kwargs)
            except Exception as e:
                raise RuntimeError("Failed to create Oracle gateway session during manual reset") from e
    ...
    def switch_session(self, session_key: str, target_session_id: str) -> Optional[SessionEntry]:
        ...
        Ends the current session in Oracle (like reset), but instead of
        ...
        if self._db and db_end_session_id:
            try:
                ...
                raise RuntimeError("Failed to end Oracle session during switch") from e

        if self._db:
            try:
                self._db.reopen_session(target_session_id)
            except Exception as e:
                raise RuntimeError("Failed to reopen Oracle session during switch") from e
    ...
    def get_transcript_path(self, session_id: str) -> Path:
        """Local transcript files are disabled in OraHermes."""
        raise RuntimeError("Local transcript files are disabled; OraHermes uses Oracle only.")
    
    def append_to_transcript(self, session_id: str, message: Dict[str, Any], skip_db: bool = False) -> None:
        """Append a message to an Oracle-backed session transcript.

        Args:
            skip_db: When True, the caller already persisted this message to
                     Oracle via its own flush path, so there is nothing to do.
        """
        if skip_db:
            return

        try:
            self._require_db().append_message(
                session_id=session_id,
            ...
            raise RuntimeError(f"Failed to append message to Oracle session {session_id}") from e
    
    def rewrite_transcript(self, session_id: str, messages: List[Dict[str, Any]]) -> None:
        """Replace the entire Oracle transcript for a session with new messages.
        
        Used by /retry, /undo, and /compress to persist modified conversation history.
        """
        try:
            self._require_db().replace_messages(session_id, messages)
        except Exception as e:
            raise RuntimeError(f"Failed to rewrite Oracle session {session_id}") from e

    def load_transcript(self, session_id: str) -> List[Dict[str, Any]]:
        """Load all messages from an Oracle-backed session transcript."""
        try:
            return self._require_db().get_messages_as_conversation(session_id)
        except Exception as e:
            raise RuntimeError(f"Failed to load Oracle session {session_id}") from e
