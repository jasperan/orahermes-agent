...
mautrix Python SDK. End-to-end encryption (E2EE) is disabled in OraHermes
until the Matrix crypto store is ported to Oracle-backed persistence.
...
Environment variables:
    ...
    MATRIX_ENCRYPTION           Must be false; E2EE storage is disabled
    MATRIX_DEVICE_ID            Reserved for future E2EE persistence
...
# Store directory reserved for future Oracle-backed E2EE state.
# Uses get_hermes_home() so each profile gets its own Matrix store.
from hermes_constants import get_hermes_dir as _get_hermes_dir

_STORE_DIR = _get_hermes_dir("platforms/matrix/store", "matrix/store")
_CRYPTO_STATE_PATH = _STORE_DIR / "crypto-state-disabled"
...
def check_matrix_requirements() -> bool:
    ...
    if encryption_requested:
        logger.error(
            "Matrix: MATRIX_ENCRYPTION=true is disabled in OraHermes because "
            "the current Matrix crypto store requires non-Oracle persistence."
        )
        return False
    ...
    try:
        ...
        if not ensure_and_bind("platform.matrix", _import, globals(), prompt=False):
            logger.warning(
                "Matrix: mautrix not installed. Run: pip install 'mautrix==0.21.0'"
            )
            return False
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        if self._encryption:
            logger.error(
                "Matrix: MATRIX_ENCRYPTION=true is disabled in OraHermes because "
                "the current mautrix crypto store requires a non-Oracle database. "
                "Disable MATRIX_ENCRYPTION or port the crypto store before using "
    ...
    async def disconnect(self) -> None:
        ...
        # Close the optional crypto store handle if a future Oracle-backed
        # Matrix E2EE implementation enables one.
