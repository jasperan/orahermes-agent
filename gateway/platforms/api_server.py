...
class ResponseStore:
    ...
    def get(self, response_id: str) -> Optional[Dict[str, Any]]:
        ...
        data = self._responses.get(response_id)
        if data is None:
            return None
    ...
    def put(self, response_id: str, data: Dict[str, Any]) -> None:
        ...
        self._responses[response_id] = json.loads(json.dumps(data, default=str))
        self._responses.move_to_end(response_id)
        while len(self._responses) > self._max_size:
            old_response_id, _ = self._responses.popitem(last=False)
            for name, stored_id in list(self._conversations.items()):
                if stored_id == old_response_id:
                    self._conversations.pop(name, None)

    def delete(self, response_id: str) -> bool:
        """Remove a response from the store. Returns True if found and deleted."""
        found = response_id in self._responses
    ...
    def set_conversation(self, name: str, response_id: str) -> None:
        ...
        self._conversations[name] = response_id

    def close(self) -> None:
        """Clear stored response state."""
...
_CORS_HEADERS = {
    ...
    "Access-Control-Expose-Headers": (
        "X-Hermes-Session-Id, X-Hermes-Session-Key, "
        "X-Hermes-Completed, X-Hermes-Partial, X-Hermes-Error"
    ),
}
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    # caller can't burn memory by passing a multi-kilobyte "session key".
        ...
        # that the sanitized form is safe to pass into Oracle state.
    ...
    def _parse_session_key_header(
        ...
        Oracle session state across transcripts.  It
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to Oracle so that ``hermes sessions list``
    ...
    def _create_agent(
        ...
        key is meant to persist across transcripts, matching the native
        gateway's ``session_key`` semantics.
        """
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # Allow caller to scope Oracle-backed session state with a stable
        # per-channel identifier via X-Hermes-Session-Key.  This
        ...
        # When provided, history is loaded from Oracle instead of from the request body.
        #
