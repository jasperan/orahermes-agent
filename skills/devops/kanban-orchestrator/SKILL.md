...
6. **The audit trail matters.** In OraHermes, use this only after the board store is ported to Oracle Database.
...
```python
t1 = kanban_create(
    title="research: Oracle cost vs current",
    assignee="<profile-A>",  # whichever profile handles research on this setup
    body="Compare estimated infrastructure costs, migration costs, and ongoing ops costs over a 3-year window. Sources: OCI pricing, team time estimates, and current Oracle bills from peers.",
    tenant=os.environ.get("HERMES_TENANT"),
)["task_id"]

t2 = kanban_create(
    title="research: Oracle performance vs current",
