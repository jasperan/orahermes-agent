...
- **Oracle-only persistence in OraHermes** — session and message state live in Oracle Database. Legacy local memory files and non-Oracle memory providers are disabled until they are ported to Oracle-backed storage.
...
hermes memory setup/status/off  Memory provider status (disabled until Oracle-backed)
...
Kanban is disabled in OraHermes until the board store is ported to Oracle
Database. Users should not rely on local DB-backed Kanban state in this fork;
dispatcher-spawned worker toolsets remain unavailable until that Oracle-backed
...
- **Isolation:** board is the hard boundary (workers get
  ...
  within a board for workspace-path isolation.
...
├── hermes_state.py       # Oracle session store
