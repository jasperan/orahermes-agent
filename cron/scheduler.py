...
def _run_job_impl(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    # Initialize Oracle session store so cron job messages are persisted
    ...
    try:
        ...
        logger.debug("Job '%s': Oracle session store not available: %s", job.get("id", "?"), e)
...
"""
        ...
        if _session_db:
            ...
            try:
                ...
                logger.debug("Job '%s': failed to close Oracle session store: %s", job_id, e)
