...
def _deliver_result(job: dict, content: str) -> None:
    ...
    if deliver == "origin":
        ...
        if origin and origin.get("platform") == platform_name:
            ...
            if not chat_id:
                logger.warning("Job '%s' deliver=%s but no chat_id or home channel. Set via: orahermes config set %s_HOME_CHANNEL <channel_id>", job["id"], deliver, platform_name.upper())
                return
